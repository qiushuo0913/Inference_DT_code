...
class TimeFreqResourceAllocationV87(Env):
    metadata = {
    ...
    def __init__(self, n_ues=32, n_prbs=25, buffer_max_size=32, eirp_dbm=13, f_carrier_mhz=2655,
        ...
        # self.min_pkt_size_bits = self.max_pkt_size_bits//2
        self.t_max = t_max
    ...
    def reset(self):
        ...
        self.tti_next_pkt = np.zeros(shape=(self.K,))
    ...
    def step(self, action):
        ...
        reward = 0
        reward_sep = np.zeros(self.K)
        self.t += 1  # Update time-step
        self.p = self.t % self.Nf  # Update PRB counter
        if self.p == 0:
            reward, reward_sep = self._calculate_reward()
        ...
        return np.array(self.state), reward, reward_sep, done, {}
    ...
    def _calculate_reward(self):
        ...
        #统计每个用户的剩余packets
        # r_gbr = np.zeros(self.K)
        r_non_gbr_sep = np.zeros(self.K)
        ...
        for u, qi in enumerate(self.qi):
            ...
            if non_gbr_delayed_pkts.size > 0:
                ...
                r_non_gbr_sep[u] = np.sum(self.s[u, non_gbr_delayed_pkts])
            if non_gbr_pkts.size > 0:
                r_non_gbr += np.sum(self.s[u, non_gbr_pkts])
                r_non_gbr_sep[u] = np.sum(self.s[u, non_gbr_pkts])

        return -r_gbr - r_non_gbr, - r_non_gbr_sep
