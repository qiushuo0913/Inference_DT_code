...
register(
    id='TimeFreqResourceAllocation-v1002',
    entry_point='wireless.envs.time_freq_resource_allocation_v102:TimeFreqResourceAllocationV87',
)
...
def main(_run):
    ...
    for ep in range(n_eps):  # Run episodes
        ...
        if _run.config['env']['env'] == 'TimeFreqResourceAllocation-v0':
            env = gym.make('TimeFreqResourceAllocation-v1002', n_ues=_run.config['env']['n_ues'],
        ...
        if ac["agent"]["agent_type"] == "proportional fair channel aware":
            CQI_0.append(env.cqi)
            s_0.append(env.s.reshape(-1))
        else:
            CQI_1.append(env.cqi)
            s_1.append(env.s.reshape(-1))
        ...
        if ac["agent"]["agent_type"] == "proportional fair channel aware":
            A_1_False.append(A)
            Y_1_False.append(-rwd[ep,-1])
        else:
            A_0_False.append(A)
            Y_0_False.append(-rwd[ep,-1])
        
        print(f"{(ep+1)*100/(n_eps):3.0f}% completed.")
            
        env.close()
        
    # np.save('../../data/CQI_0.npy', CQI_0)   
    # np.save('../../data/CQI_1.npy', CQI_1)
    # np.save('../../data/s_0.npy', s_0)   
    # np.save('../../data/s_1.npy', s_1)
    
    np.save('../../data/Y_0_false.npy', Y_0_False)   
    np.save('../../data/Y_1_false.npy', Y_1_False)
    
    np.save('../../data/A_0_false.npy', A_0_False)   
    np.save('../../data/A_1_false.npy', A_1_False)
